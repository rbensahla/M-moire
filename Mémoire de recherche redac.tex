...
\begin{document}
   ...
   \section{Introduction}
	...
	\subsubsection{Représentation de Mandelbrot-Van-Ness}
	  ...
	  \[ B^H(t) := \frac{1}{C(H)}\int_{\mathbb{R}} [ ((t-s)^+)^{H-\frac{1}{2}}-((-s)^+)^{H-\frac{1}{2}}\ ]\mathrm{d}B(s),\] 
	  Avec : $C(H) = \left(  \int_{0}^{\infty} \left((1+s)^{H-\frac{1}{2}} - s^{H - \frac{1}{2}} \right)^2ds + \frac{1}{2H}      \right)^{\frac{1}{2}}$
	  
	  Nous allons démontrer ce théorème, selon Mandelbrot-Van-Ness : \newline
	  On définit $f_t(s) = ((t-s)^+)^{H-\frac{1}{2}} - ((-s)^+)^{H-\frac{1}{2}}, \;\;\; s \in \mathbb{R}, t \geq 0$ \newline
	  ...
	  On a par l'isométrie d'Itô : 
	  \[ \mathbb{E}[X_t^2] = \int_{\mathbb{R}} \left[ ((t-s)^+)^{H-\frac{1}{2}} - ((-s)^+)^{H-\frac{1}{2}}   \right]^2ds \]
	  Pour $t > 0$, le résultat étant trivial pour $t = 0$, en factorisant par $t$, on a : 
	  \[ = t^{2H}\int_{\mathbb{R}} \left[ ((1-u)^+)^{H-\frac{1}{2}} - ((-u)^+)^{H-\frac{1}{2}}              \right]^2du\]
	  On décompose l'intégrale sur $\mathbb{R}^-$ et sur $\mathbb{R}^+$, on a :
	  \[ = t^{2H}     \left(\int_{-\infty}^{0} [(1-u)^{H-\frac{1}{2}} - (-u)^{H-\frac{1}{2}}]^2du + \int_{0}^{1}(1-u)^{2H-1}du             \right)  \]
	  \[ = C(H)^2t^{2H}\]
	  On a aussi, pour $s < t$ : 
	  \[ \mathbb{E}[\vert X_t - X_s \rvert^2] = \int_{\mathbb{R}} \left[  ((t-u)^+)^{H - \frac{1}{2}} - ((s-u)^+)^{H - \frac{1}{2}}    \right]^2du \]
	  \[ \mathbb{E}[\vert X_t - X_s \rvert^2] = \int_{\mathbb{R}} \left[  ((t-s-u)^+)^{H - \frac{1}{2}} - ((-u)^+)^{H - \frac{1}{2}}    \right]^2du \]
	  \[ \mathbb{E}[\vert X_t - X_s \rvert^2] = C(H)^2\lvert t-s \rvert^{2H} \] \newline
	  ...
	  Démontrons cette précédente affirmation, l'idée étant de montrer que si le mouvement brownien fractionnaire est une semi-martingale alors la variation quadratique doit être finie. : 
	  Posons $p > 0$ : 
	...
	\end{itemize} 
	  ...
	  \subsection{Simulation du mouvement brownien fractionnaire} 
	...
	Dans l'article de Gatheral, Jaisson et Rosenbaum, un modèle plus complexe est proposé, le Rough Fractional Stochastic Volatility (RFSV), celui-ci est le même que le modèle FSV mais en prenant $H \in ]0; \frac{1}{2}[$, nous détaillerons le modèle RFSV dans la section 4.5.\newline \newline
	...
	On remarque que le paramètre H varie entre 0.071 et 0.149, de plus on remarque que la variation de l'estimation du paramètre H depuis la simulation effectuée par Rosenbaum et Gatheral (En Mai 2016, cf Figure 5) n'a que très peu variée, ce qui confirme l'affirmation de la constance du paramètre H dans le temps.
	Enfin, cela permet aux praticiens d'éviter à estimer H souvent et contrairement à d'autres problématiques qui nécessitent de réajuster le calibrage tous les jours (Par exemple la calibrage de la volatility smile).
	
	\begin{figure}[!h]
  		\includegraphics[width=\linewidth, height=150mm]{SimRosenbaum.png}
  		\caption{Estimation du paramètre H de différents indices, en Mai 2016, par Gatheral et Rosenbaum}
 		\label{fig:FigB}
	\end{figure}
	
   \section{EDS dirigées par un mouvement brownien et processus d'Ornstein-Uhlenbeck fractionnaire}
   ...
   Enfin, nous parlerons des équations dirigés par le mouvement brownien fractionnaire tels que le processus d'Ornstein-Uhlenbeck, le modèle CIR ou encore le modèle de Black Scholes fractionnaire.
   	\subsection{Présentation sous la forme différentielle et sous la forme intégrale}
	...
	\subsection{Schéma numérique de Milstein}
	
		Nous allons définir le schéma de Milstein selon Unal et Dinler.\newpage 
	...
	\subsection{Processus construits sur le mouvement brownien fractionnaire}
		\subsubsection{Linéarisation d'équations différentielles stochastiques unidimensionnelle}
			Pour pouvoir avancer dans la suite de notre étude (pour pouvoir donner une solution exacte du Processus d'Ornstein-Uhlenbeck fractionnaire) nous avons besoin d'introduire, suivant la présentation de Unal et Dinler, une méthode de construction de solutions théoriques. \newline
			\paragraph{Théorème 1} Posons : 
			\[ dX_t = f(X_t,t)dt + g(X_t,t)dB_t^H  \tag{1} \] 
			Où $X_0 = x_0, t \geq 0, H \in (0,1)$ et $dB_t^H$ l'incrément du mouvement brownien fractionnaire $B^H_t$ , avec f et g, deux fonctions supposées assez régulières, l'équation (1) peut s'écrire sous la forme linéaire suivante : 
			\[ dY_t = (a(t)Y_t + b(t))dt + (c(t)Y_t + e(t))dB^H_t \tag{2} \] 
			...
			\paragraph{Définition 1}
				La fonction $F=F(t, B^H_t)$ vérifiant $d(Fy) = D_1(t,B^H_t)dt + D_2(t,B^H_t)dB^H_t$ est un facteur intégrant pour des équations différentielles stochastiques linéaires unidimensionnelles, on peut donc écrire : 
				\[ F(t, B^H_t) = exp \left(- \int_{}^{t} c(s)dB^H_s + H \int_{}^{t}s^{2H - 1}c^2(s)ds - \int_{}^{t} a(s)ds \right) \] 
				Par suite, on a donc la solution de l'équation différentielle stochastique linéaire suivante :
				\[ Y_t = \frac{1}{F}\left( \int_{}^{t} F(s)b(s)ds - 2H \int_{}^{t} s^{2H-1}F(s)c(s)e(s)ds + \int_{}^{t}F(s)e(s)dB^H_s+y_0 \right) \] 
		...
		\[ Y_t = e^{\int_{}^{t} \theta(s)ds}\left(  \int_{}^{t} e^{\int \theta(s)ds} \theta(s)\mu(s)ds + \int_{}^{t} e^{\int \theta(s)ds} \sigma(s)dB^H_s +y_0 \right) \tag{3}\] \newline
		...
		On constate bien le comportement caractéristique du processus d'Ornstein-Uhlenbeck qui n'est autre que le retour à la moyenne (mean-reverting).
	\section{Les modèles fractionnaires (Rough Model)}
		\subsection{Modèle de Cox-Ingersoll-Ross fractionnaire}
			Le modèle CIR dirigé par un mouvement brownien fractionnaire est défini par la dynamique suivante : 
			\[ dX_t = \gamma(t)(\theta(t)-X_t)dt + \sigma(t)\sqrt{x}dB^H_t \]
			Ce modèle est utilisé afin de modéliser l'évolution de taux d'intérêts. \newline
			On a selon Unal et Diner : $f(x,t) = \gamma(t)(\theta(t)-x)$, $g(x,t) = \sigma(t)\sqrt{x}$. \newline On a donc $L = \frac{1}{2} \frac{\sigma(t)}{\sqrt{x}} + \frac{\sigma(t)}{\sigma(t)} \frac{-1}{2}x^{-\frac{3}{2}}\theta(t) - \frac{1}{2}x^{-\frac{1}{2}} -Ht^{2H-1}\frac{-\sigma(t)}{4}x^{-\frac{3}{2}}$ \newline \newline
			...
			\[ \left((\int \alpha(t)dt)^\prime + (\frac{\gamma(t)}{2} + \sigma(t) \frac{\partial}{\partial t}\frac{1}{\sigma(t)} \right) \int \alpha(t)dt  = 0 \]
			...
			\[ dY_t = e^{\int \frac{\gamma(t)}{2}}\sigma(t)dB^H_t \]			
			On obtient la solution en intégrant :
			\[ Y_t = \int_{0}^{t}e^{\int \frac{\gamma(t)}{2}}\sigma(t)dB^H_t + K \] \newline
			Enfin, on obtient donc la solution du modèle CIR : 
			\[ X_t = \left(\frac{1}{2}exp\left(-\int \frac{1}{2} \gamma(t)dt \right) \left(\int_{0}^{t}exp\left(\int \frac{1}{2}\gamma(t)dt \right) \sigma(t)dB^H_t + \sqrt{x(0)} \right) \right)^2  \]
		...
		\subsection{Le Modèle de Black-Scholes fractionnaire}
		...
		\subsection{Modèle Rough Heston}
			...
			\subsubsection{Construction du modèle Rough Heston}
				...
				Finalement, la loi du processus conditionnellement à $F_t$, $(S_t^{t_0}, V_t^{t_0})_{t \geq 0} = (S_{t+t_0}, V_{t+t_0})_{t \geq 0}$ est la loi du modèle Rough Heston dont les dynamiques de volatilités et de prix sont les suivants : 
				...
				Comme l'on démontré El Euch et Rosenbaum, il est nécessaire d'utiliser le théorème de Fubini stochastique, grâce à ce dernier on obtient que $I^{1-\alpha}V$ est une semi-martingale pour $t > 0$, en effet :
				\[ (I^{1-\alpha}V)_t  =  \frac{1}{\Gamma(1-\alpha)} \int_{0}^{t}\frac{V_s}{(t-s)^{\alpha}}ds \]
				On remplace $V_t$ par son expression donnée ci-dessus, et on utilise le théorème de Fubini stochastique et on en déduit que : 
				\[  (I^{1-\alpha}V)_t = V_0\int_{0}^{t}\frac{s^{-\alpha}}{\Gamma(1-\alpha)}ds + \int_{0}^{t} \lambda(\theta^0(s) - V_s)ds + \int_{0}^{t}\nu\sqrt{V_s}dB_s \]
				On a donc : 
				\[ (I^{1-\alpha}V)_{t+t_0} = \frac{1}{\Gamma(1-\alpha)} \int_{0}^{t+t_0}(t+t_0 - u)^{-\alpha} V_udu \]
				qui vaut :
				...
				Pour finir nous avons : 
					\[ \int_{0}^{t}V_s^{t_0}ds = \frac{1}{\Gamma{(\alpha)}} \int_{0}^{t}(t-s)^{\alpha - 1}I^{1-\alpha}V_s^{t_0}ds \]
				qui vaut :
				...
				\subsubsection{Schéma numérique de calcul de la fonction caractéristique du log-prix dans le modèle Rough Heston}
					...
					Avec une interpolation linéaire de la fonction $g$ on obtient : 
					...
					On calcule ensuite une pré-estimation de $\hat{h}(a, t_{k+1})$ basée sur une somme de Riemann 
			...
			\subsection{Le modèle rough SABR}
				...
				\subsubsection{Le modèle rough SABR}
					Nous ne rentrerons pas dans des considérations techniques compliquées, selon Gatheral et Fukasawa la dynamique du modèle rough SABR est donnée par : 
					\[ \frac{dS_t}{\beta(S_t} = \alpha_tdZ_t, \;\; \frac{d \xi(s)}{\xi(s)} =  \kappa(s-t)dB_t^H\] sous une mesure-martingale équivalente $Q$, avec $\alpha_t = \sqrt{\xi(t)}$ et $\beta$ une fonction continue et positive. \newline
					(Z, B) est un mouvement $F_t$brownien bi-dimensionel corrélé, tel que $d \langle Z, B \rangle_t  = \rho dt, \kappa(t) = \eta \sqrt{2Ht^{H-1/2}}, \; \rho \in [-1;1], \eta > 0$ et $H \in (0, \frac{1}{2}]$, on suppose que $(\xi_0(s))_{s\leq0}$ est une famille de variable aléatoire $F_0-$mesurable et que la courbe $s \rightarrow \xi(s)$ 					est continue.
					
			\subsection{L'indice VIX}
				\subsubsection{Introduction}
					Le VIX est un indicateur de volatilité de marché financier. \newline
					Bayer, Friz et Gatheral ont proposé un modèle adapté au processus log-prix $X_t:=log(S):$, la dynamique de leur modèle est la suivante :
					\[ dX_t = -\frac{1}{2}V_tdt + \sqrt{V_t}dB_t, \;\; X_0 = 0 \]
					\[  V_t = \xi_0(t)\epsilon(2 \nu C_H\upsilon_t), \;\;\; V_0 > 0,  \]
					Le processus $\upsilon$ défini par : 
					\[ \upsilon_t := \int_{0}^{t}(t-u)^H_{-} \]
					Avec $H_{+} = H + \frac{1}{2}$ et $H_{-} = H - \frac{1}{2}$ \newline
					Est un processus Gaussien avec une fonction de covariance :
					\[ \mathbb{E}[\upsilon_t \upsilon_s] = s^{2H}\int_{0}^{1} \left(\frac{t}{s} - u    \right)^H_- (1-u)^H_-du, \;\;\;  pour tout s,t\in[0,1] \]
					De plus, on définit la volatilité moyenne par : 
					\[ \sigma_T^2(t) = \frac{1}{T-t}\int_{t}^{T}\xi_T(u)du,\]
				\subsubsection{VIX Futures sur le modèle rough Bergomi}
					On définit le VIX au temps T par : 
					\[ VIX_T^2 := \mathbb{E} \left[    \frac{1}{\Delta}\int_{T}^{T+\Delta}d \langle    X_s, X_s     \rangle ds   \rvert F_t \right] \]
					\[ \mathbb{B}_T := \mathbb{E}[VIX_T \rvert F_0]  = \mathbb{E} \left[\sqrt{\frac{1}{\Delta}\int_{T}^{T+\Delta}d \langle    X_s, X_s     \rangle ds } \rvert F_0 = \mathbb{E} \left[   \sqrt{\frac{1}{\Delta}\int_{T}^{T+\Delta \xi_T(s)ds}}  \rvert F_0 \right]  \right] \]
					Lorsque $T > 0$, $\xi_T(s)$ est n'est pas $F_0-$mesurable, nous allons utiliser cette variable à plusieurs reprises : 
					\[\eta_T(t) := exp(2\nu C_H\upsilon_t^T)\]
					La dynamique est donnée par : 
					\[ VIX_T = \left( \frac{1}{\Delta}\int_{T}{T+\Delta}\xi_0(t)\eta(t)exp \left(   \frac{\nu^2 C_H^2}{H}[(T-t)^{2H} - t^{2H}]  \right)dt     \right) ^{1/2} \]
					La démonstration est donnée dans le papier de Jacquier, Martini et Muguruza. "ON VIX FUTURES IN THE ROUGH BERGOMI MODEL".
					La courbe de variance forward $\xi_t$ dans le modèle Rough Bergomi admet la représentation suivante : 
					\[ \xi_T(t) = \xi_0(t)\eta_T(t)exp  \left(    \frac{\nu^2 C_H^2}{H}[(T-t)^{2H} - t^{2H}]    \right) \]
					Cette démonstration est aussi donnée dans le papier précédent.
					
				
					
			\subsection{Le modèle RFSV}
				\subsubsection{Définition du modèle}
				Gatheral et Rosenbaum ont montré empiriquement, que la distribution des incréments de la log-volatilité se rapprochée de la distribution Gaussienne et que ces incréments possédaient une propriété d'échelle, ainsi ils proposent le modèle suivant :
				\[ log \sigma_{t+\Delta} - \log \sigma_t  = \nu (B^H_{t+\Delta} - B^H_t),\]
				Avec $B^H_t$ un mouvement brownien fractionnaire, $\nu > 0$ et $H$ l'index de Hurst mesuré empiriquement.
				La volatilité peut donc s'écrire : 
				\[ \sigma_t = \sigma exp(\nu W^H_t)  \]
				Ce modèle n'étant pas stationnaire cela amène à considérer non plus la log-volatilité comme un mouvement brownien fractionnaire, mais comme un processus d'Ornstein-Uhlenbeck fractionnaire 
				Un processus d'Ornstein-Uhlenbeck stationnaire est simplement défini comme la solution stationnaire de l'équation différentielle stochastique suivante : 
				\[ dX_t = \nu dB_t^H - \alpha(X_t-m)dt, \]
				Avec $m \in \mathbb{R}$ et $\nu$ et $\alpha$ sont des paramètres positifs, bien entendu la forme explicite de la solution du processus d'Ornstein-Uhlenbeck est donnée par : 
				\[ X_t = \nu \int_{-\infty}^{t}e^{-\alpha(t-s)}dB_t^H + m \] \newpage
				Ainsi, sur $[0; T]$ on a maintenant : 
				\[ \sigma_t = exp(X_t) \, \;\;\; t \in [0;T]\]
				Avec : $\nu > 0, \alpha > 0, m\in \mathbb{R}$ et $H < \frac{1}{2}$, ce modèle est stationnaire, de plus lorsque $\alpha << 1/T$ la log-volatilité se comporte (localement) comme un mouvement brownien fractionnaire, de plus, selon Gatheral et Rosenbaum sous certaines conditions,  lorsque $\alpha$ tends vers 0 on a : 
				...
				\subsubsection{Prévisions de la volatilité dans le modèle RFSV}
					Voyons maintenant une formule très importante permettant la prévision de la log-volatilité, il s'agit de la formule de Neuzmann-Poor utilisé dans l'étude de Rosenbaum et Gatheral, celle-ci est donnée plus généralement par :
					\[ \mathbb{E}[B_{t+\Delta}^H \rvert F_t] = \frac{cos(H\pi)}{\pi}\Delta^{H+1/2}\int_{-\infty}^{t}\frac{B_s^H}{(t-s+\Delta)(t-s)^{H+1/2}}ds, \]
					Où : $B^H_t$ est un mouvement brownien fractionnaire avec un paramètre de Hurst $H < 1/2$ et $F_t$ la filtration engendrée.\newline
					Par la suite, on va approximer le processus de volatilité ($\sigma_t$) d'Ornstein-Uhlenbeck par : $2\nu B_t^H + C$, avec $C$ et $\nu$ deux constantes, la formule de prédiction de la volatilité est donc donnée par :
					...
					Dans l'étude de Rosenbaum et Gatheral, cette formule est utilisé afin de prévoir la log-volatilité à 1, 5 et 20 jours.
   ...
   \newpage
   \section*{Conclusion}
   Aujourd'hui tous les établissements financiers ont l'habitude de modéliser les prix d'actifs par des semi-martingales par rapport au mouvement brownien classique pour le prix des couvertures d'options et de divers contrats, en pratique les experts de la finance utilisent des modèles plus complexes que le modèle de Black-Scholes, tels que : Des modèles à volatilité, locale ou combinaison des deux local/stochastique \newline
L'objectif est de répliquer au mieux les aspects empiriques des marchés financiers, les Modèles CEV, SABR, Heston et Bergomi qui sont les plus utilisés en pratique pour leur facilités : Formule fermées pour prix et couverture, méthodes de Monte-Carlo issues de l'analyse numérique, ces modèles présentent leur avantages et leurs inconvénients, par exemple : \newline Les modèles à volatilité locale pour les options vanilles, mais mauvaise volatilité implicite, volatilité stochastique pour le pricing d'option exotique, notamment les paths-dependents ou encore SABR pour les marchés FOREX, ces modèles à volatilité stochastique génèrent une bonne surface de volatilité mais ne permettent pas d'avoir un bon calibrage du skew-volatility à la monnaie (qui est la dérivée de la volatilité implicite par rapport au strike), cependant ces modèles n'arrivent pas à reproduire tous les aspects empiriques des marchés financiers. \newline
On a vu dans ce mémoire une nouvelle aproche proposée par Rosenbaum et Gatheral qui établit le fait que la log-volatilité est en réalité rugueuse, c'est-à-dire dirigée par un mouvement brownien fractionnaire de paramètre de Hurst $H < \frac{1}{2}$, de nombreux chercheurs ont alors essayé d'adapter ce nouveau fait établit en construisant de nouveaux modèles basés sur cette hypothèse.\newline
Malheureusement, comme nous avons pu le voir le fait que le mouvement brownien fractionnaire ne soit ni une martingale, ni un processus Markovien, rend les choses compliqués puisque nous n'avons pas encore les moyens techniques de définir proprement les intégrales stochastiques pour $H < \frac{1}{2}$ et en déduire des formules de changement de variables type Itô.\newline
De plus, le côté non-markovien du mouvement brownien fractionnaire rend difficile le calcul de l'espérance conditionnelle nécessaire au pricing d'option.\newline
De nouvelles constructions comme celle du Rough Heston, Rough SABR, Rough Bergomi ont montré une réelle efficacité pour répondre aux problématiques soulevés précédemment, de plus, la prévision de volatilité est surprenante puisqu'elle colle très bien aux données empiriques (cf Figure 10 , de Rosenbaum et Gatheral). \newline
		\begin{figure}[H]
  			\includegraphics[width=\linewidth, height=80mm]{GatheralVolatilite.png}
  			\caption{Prévision de volatilité en rouge; données empiriques en bleu, graphique de Jim Gatheral}
 			\label{fig:figD}
		\end{figure}
Il est aussi très important de remarquer que dans les modèles Rough cités précédemment on peut obtenir des informations très importantes des formules fermées ainsi que certaines lois conditionnelles qui permettent d'en déduire par exemple un hedging optimal. \newline
La perspective apportée par le mouvement brownien fractionnaire paraît très intéressante et semble susciter l'intérêt de plusieurs grandes compagnies.\newline
Enfin, c'est un domaine de recherche extrêmement actif qui a montré son efficacité en pratique, il convient de noter néanmoins que toute cette approche théorique est assez compliquée et il faudra certainement du temps avant que la large majorité des institutions financières adoptent définitivement cette théorie.
 \newpage
   ...
   \section{L'approche de Carr et Madan}
	...
	Si on connait la densité du processus $S_t$ on peut utiliser la formule de transfert pour calculer le prix du Call, ici $L=e^k$ et donc :
	\begin{align}
	...
	On peut en déduire une formule similaire pour le put, on peut donc répliquer de manière théorique, toutes fonctions en utilisant uniquement des Calls et des Puts (Continuum Calls/Puts).
	On écrit donc : 
	\[ f(S_T) = f(\kappa) + f^\prime(\kappa)(S_T-\kappa) +  \int_{-\infty}^{\kappa}f^{\prime\prime}(K)(K-S_T)^+dK + \int_{\kappa}^{\infty}f^{\prime\prime}(K)(S_T-K)^+ dK\]
	Elle est parfois utilisée pour le pricing variance swap sur l'indice VIX.
   ...
   \begin{thebibliography}{99}
	...
	\bibitem{C}
	{Martin Schmelze} {\textbf{\em Option Pricing Formulae using Fourier Transform: Theory and Application }}
	
	\bibitem{C}
	{Antoine Jacquier, Claude Martini, Aitor Muguruza} {\textbf{\em ON VIX FUTURES IN THE ROUGH BERGOMI MODEL }}
