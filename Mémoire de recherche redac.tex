	...
	\subsection{Modèles et Processus construits sur le mouvement brownien fractionnaire}
		...
		\subsection{Modèle Rough Heston}
			...
			\subsubsection{Modèle de Heston dirigé par un mouvement brownien fractionnaire}
				...
				\[ \partial_t h = \frac{1}{2}(-a^2 - ia) + \lambda(ia\rho \nu - 1)h(a,s) +  \frac{(\lambda \nu)^2}{2}h^2(a,s), \;\;\; h(a,0) = 0 \]
				\[ L(t,\lambda V_tS_t) = \mathbb{E}[e^{i\lambda log(S_t)} \lvert f_t]\] \newline
				...
				\[ \partial L = \left(\lambda (\theta - V)\frac{\partial L}{\partial V} + \frac{1}{2} (\lambda v)^2 V \frac{\partial^2 v}{\partial L} + \frac{1}{2}s^2v\frac{\partial L}{\partial S} + \rho V \lambda sV*\frac{\partial^2 SV}{\partial L}  = 0 \right)\] 
			\subsubsection{Construction du modèle Rough Heston}
				...
				On voudrait un modèle qui prend en compte l'aspect fractionnaire des marchés et qui reproduit les propriétés importantes du modèle de Heston :
				\begin{itemize}
					...
					\item La modélisation permet de reproduire la dynamique du volatility smile (Dépendance de la volatilité implicite par rapport au Strike $K$), comme prévu par le modèle de Black-Scholes on remarque que la volailité implicite n'est pas constante mais dépendante du strike $K$
				\phantom{a}
				\begin{figure}[h!]
				...
				\newpage		
				\end{itemize}
				En pratique le modèle de Heston, et donc le modèle Rough Heston est utilisé afin de pricer des options exotiques, mais cette modélisation tend à créer une divergence entre le pricing d'options vanilles (faussés) et la modélisation de ces derniers par un modèle à volatilité constante/déterministe (plus réel).
